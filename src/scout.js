//     scout.js
//     Copyright (c) 2013-2014 iconmobile LLC
...
(function(exports) {
    var scout = function() {
        ...
        $.fn = {
            ...
            serialize: function() {
                var s = "";
                function append(s, item) {
                    if(item instanceof Scout || Array.isArray(item)) {
                        for (var i = 0; i < item.length; i++)
                            s = append(s, item[i]);
                        return s;
                    }
                    else if (item.tagName==="FORM") {
                        return append(s, $(item).find("input,select,textarea,button"));
                    }
                    else if(item.name) {
                    	var val = encodeURIComponent(
                    		item.type.toLowerCase() == "checkbox" ? !!item.checked : item.value);
                        return s += (s.length > 0 ? "&" : "")
    ...
    if (typeof exports.define !== "undefined" &&
        exports.define !== null ? exports.define.amd : void 0) {
        return exports.define('scout', [], function() { return scout(); });
    } 
})(window);
//     ajax.js
//     Copyright (c) 2013-2014 iconmobile LLC
    ...
    var extend = function($) {
        $.ajax = function(url, options) {
            ...
            if (!('withCredentials' in xhr) && window.XDomainRequest && isCORS) {
                ...
                if (xhr) {
                    xhr.onload = function() {
                        options.success.call(options.context, xhr.responseText);
                ...
                xhr.onreadystatechange = function() {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300 || xhr.status == 304) {
                        options.success.call(options.context, xhr.responseText, xhr.getResponseHeader('Content-Type'));
                        options.complete.call(options.context, xhr.responseText, xhr.getResponseHeader('Content-Type'));
                        deferred.resolve(xhr.responseText, xhr.statusText, xhr);
                    } else if (xhr.status >= 400) {
                        options.error.call(options.context, xhr.responseText, new Error(xhr.status));
